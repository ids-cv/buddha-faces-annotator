...
def auto_call(im_path, bbox=None):
    ...
    tmp = []
    for index, val in enumerate(new_preds['jaw_line']):
        if index % 2 == 0:
            tmp.append(val)
    new_preds['jaw_line'] = tmp
    tmp = []
    new_preds['right_eye'] = new_preds['right_eye'][1:-1]
    for index, val in enumerate(new_preds['right_eye']):
        if index % 2 == 0:
            tmp.append(val)
    new_preds['right_eye'] = tmp
    tmp = []
    new_preds['right_eyebrow'] = [new_preds['right_eyebrow'][0]] + [new_preds['right_eyebrow'][2]] + new_preds['right_eyebrow'][4:]
    new_preds['left_eye'] = new_preds['left_eye'][1:-1]
    for index, val in enumerate(new_preds['left_eye']):
        if index % 2 == 0:
            tmp.append(val)
    new_preds['left_eye'] = tmp
    new_preds['left_eyebrow'] = [new_preds['left_eyebrow'][0]] + [new_preds['left_eyebrow'][2]] + new_preds['left_eyebrow'][4:]
    new_preds['nose'] = new_preds['nose'][0:3] + [new_preds['nose'][4]] + new_preds['nose'][6:]
    return new_preds
